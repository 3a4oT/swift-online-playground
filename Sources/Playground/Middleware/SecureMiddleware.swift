...
public final class SecureMiddleware: Middleware, ServiceType {
    
    
    public static func makeService(for container: Container) throws -> SecureMiddleware {
        return .init()
    }
    ...
    public func respond(to request: Request, chainingTo next: Responder) throws -> Future<Response> {
        ...
        return response.map { response in
            ...
            if let xssProtection = self.configuration.xssProtection, !xssProtection.description.isEmpty {
                response.http.headers.replaceOrAdd(name: "X-XSS-Protection", value: xssProtection.description)
            }
            
            if let xContentTypeOptions = self.configuration.xContentTypeOptions, !xContentTypeOptions.description.isEmpty {
                response.http.headers.replaceOrAdd(name: "X-Content-Type-Options", value: xContentTypeOptions.description)
            }
            
            if let xframeOptions = self.configuration.xframeOptions, !xframeOptions.description.isEmpty {
                response.http.headers.replaceOrAdd(name: .xFrameOptions, value: xframeOptions.description)
            }
            
            if let strictTransportSecurity = self.configuration.strictTransportSecurity, !strictTransportSecurity.description.isEmpty {
                response.http.headers.replaceOrAdd(name: .strictTransportSecurity, value: strictTransportSecurity.description)
            }
            
            if let contentSecurityPolicy = self.configuration.contentSecurityPolicy, !contentSecurityPolicy.description.isEmpty {
                response.http.headers.replaceOrAdd(name: "Content-Security-Policy", value: contentSecurityPolicy.description)
            }
